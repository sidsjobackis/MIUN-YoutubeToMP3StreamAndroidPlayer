        ...
        lv_playlists.setOnItemClickListener(new ListView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> a, View v, int i, long l) {
                try {
                    ...
                    RestClient clientFetchUserPlaylist = new RestClient( 
                    													PlayerActivity.BASE_URL 
                    													+ String.valueOf(itemClickedIndex)); 
                    
                    //clientFetchUserPlaylist.AddParam( "index", String.valueOf(itemClickedIndex) );
                	
                	//Call Webservice fetchUserPlaylist(int index)
            		try {
            			clientFetchUserPlaylist.Execute(RequestMethod.GET);
                	} catch (Exception e) {
                	    e.printStackTrace();
                	}
                	    	    	   
                	//Take returned JSON/GSON object and populate the playlist
                	PlayerActivity.USERPLAYLISTS.getPlaylist(itemClickedIndex).populate(clientFetchUserPlaylist.getResponse());
